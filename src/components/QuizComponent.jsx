...
// wpisanie punktów do tablicy wyników
const handleAnswerPoints = (isCorrect) => {
    const newResult = isCorrect ? 1 : 0;
    if (!arrayPoints[currentIndex]) {
        setArrayPoints([...arrayPoints, newResult]);
    }
}
...
const handleCheckAnswer = (chosenOption, key) => {
    ...
    if (currentQuestion.correct_answer === chosenOption && !arrayPoints[currentIndex]) {
        const points = currentPoints + 1;
        setPoints(points);
        changePermission(false);
        markAnswer({key, variant: 'bg-success'});
        handleAnswerPoints(true);
    } else {
        if(arrayPoints[currentIndex]) {
            const points = currentPoints ;
            setPoints(points < 0 ? 0 : points);
            markAnswer({key, variant: 'bg-danger'});
            handleAnswerPoints(true);
        } else {
            changePermission(false);
            markAnswer({key, variant: 'bg-danger'});
            handleAnswerPoints(false);
        }
    }
};
